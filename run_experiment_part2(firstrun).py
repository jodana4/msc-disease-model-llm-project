...
LLM:claude-sonnet-4-6-agentic[{today}]
...
def main():
    ...
    for omim_id, disease_name in DISEASES:
        ...
        # Wait before every request including the first
        time.sleep(180)

        # Skip if already processed
        safe_name = omim_id.replace(':', '_')
        output_path = f'outputs/experiment/part2/summaries/{safe_name}.txt'

        if os.path.exists(output_path):
            print(f"  Skipping {omim_id} — already processed")
            continue
        ...
        try:
            ...
            print(f"  Done — saved to outputs/experiment/part2/")
    ...
    print("\nAll done — combined file saved to outputs/experiment/part2/all_diseases_part2.tsv")
