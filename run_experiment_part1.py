...
IMPORTANT: Do NOT search the web or look up any external sources. 
Use only your existing training knowledge to complete this task.

STEP 1 — IDENTIFY HALLMARK PHENOTYPES
From your existing knowledge, identify phenotypes that are hallmarks. 
A hallmark phenotype is a phenotype that is present in 80% or more of patients with this disease. 
Only include phenotypes you are confident about from your training data.

STEP 2 — ASSIGN HPO TERMS
For each hallmark phenotype, assign the most specific HPO term ID you 
know. Use the format HP:XXXXXXX.

STEP 3 — ASSIGN FREQUENCY
Assign frequency using HPO frequency terms:
...
--- PHENOTYPE SUMMARY ---
HP:XXXXXXX | Term Name | Frequency
--- END SUMMARY ---
...
- Do NOT search the web, use training knowledge only
...
def main():
    ...
    os.makedirs('outputs/experiment/part1/summaries', exist_ok=True)
    os.makedirs('outputs/experiment/part1/hpoa_rows', exist_ok=True)
    all_hpoa_rows = []
    ...
    for omim_id, disease_name in DISEASES:
        ...
        try:
            ...
            # Allows saving full output including summary
            safe_name = omim_id.replace(':', '_')
            with open(f'outputs/experiment/part1/summaries/{safe_name}.txt', 'w') as f:
                f.write(f"Disease: {disease_name} ({omim_id})\n")
                f.write("=" * 60 + "\n")
                f.write(output)

            # Extract just the HPOA rows (after END SUMMARY line)
            ...
            with open(f'outputs/experiment/part1/hpoa_rows/{safe_name}.tsv', 'w') as f:
                f.write(hpoa_section)

            # Collect for combined file
            all_hpoa_rows.append(hpoa_section)

            print(f"  Done — saved to outputs/experiment/part1/")
    ...
    # Save combined HPOA file with all diseases
    with open('outputs/experiment/part1/all_diseases_part1.tsv', 'w') as f:
        f.write('\n'.join(all_hpoa_rows))

    print("\nAll done — combined file saved to outputs/experiment/part1/all_diseases_part1.tsv")
